...
void ServerGameLogic::ApiPlayerUpdateChar(obj_ServerPlayer* plr, bool disconnectAfter)
{
	...
	//if (plr->loadout_->GameMapId == GBGameInfo::MAPID_California) return;
...
void ServerGameLogic::GetStartSpawnPosition(const wiCharDataFull& loadout, r3dPoint3D* pos, float* dir,DWORD peerId)
{
	...
	if ( loadout.GameMapId != GBGameInfo::MAPID_UB_Mega || loadout.GameMapId != GBGameInfo::MAPID_UB_Valley || loadout.GameMapId != GBGameInfo::MAPID_UB_Area51 || loadout.GameMapId != GBGameInfo::MAPID_UB_CryZ /*|| loadout.GameMapId != GBGameInfo::MAPID_UB_Terra*/)
	{
...
int ServerGameLogic::ProcessChatCommand(obj_ServerPlayer* plr, const char* cmd)
{
	...
	if(plr->profile_.ProfileData.isDevAccount)
	{
		...
		/*	if(strncmp(cmd, "/invite", 5) == 0)
		return Cmd_KickGroupI(plr, cmd);

		if(strncmp(cmd, "/joingroup", 5) == 0)
		return Cmd_KickGroupJ(plr, cmd);*/
	}
	return 1;	
}
...
/*int ServerGameLogic::Cmd_KickGroupI(obj_ServerPlayer* plr, const char* cmd)
{
char buf[128];
char find[64];
if(2 != sscanf(cmd, "%s %63c", buf, find))
...
char message[64] = {0};
sprintf(message, "Player '%s' has invite you", (plr->userName));
PKT_C2C_ChatMessage_s n2;
n2.userFlag = 0;
n2.msgChannel = 0;
r3dscpy(n2.msg, message);
r3dscpy(n2.gamertag, "<system>");
p2pSendToPeer(tplr->peerId_, tplr, &n2, sizeof(n2));
r3dOutToLog("Player %s has invite %s", plr->userName, tplr->userName);
...
*/
/*int ServerGameLogic::Cmd_KickGroupJ(obj_ServerPlayer* plr, const char* cmd)
{
char buf[128];
char find[64];
if(2 != sscanf(cmd, "%s %63c", buf, find))
...
char message[64] = {0};
sprintf(message, "Player '%s' has join group", (plr->userName));
PKT_C2C_ChatMessage_s n2;
n2.userFlag = 0;
n2.msgChannel = 0;
r3dscpy(n2.msg, message);
...
PKT_C2C_GroupJoin_s n;
n.isLegend = false;
//n.gamertag = (plr->userName);
//r3dscpy(n3.msg, message);
r3dscpy(n.gamertag, plr->userName);
p2pSendToPeer(tplr->peerId_, tplr, &n, sizeof(n));
p2pSendToPeer(plr->peerId_, tplr, &n, sizeof(n));
r3dOutToLog("Player %s has join group %s", plr->userName, tplr->userName);
...
IMPL_PACKET_FUNC(ServerGameLogic, PKT_C2S_UseNetObject)
{
	...
	//gServerLogic.ApiPlayerUpdateChar(fromPlr); //Codex Vault Desativar temporario
...
void ServerGameLogic::OnPKT_C2S_ScreenshotData(DWORD peerId, const int size, const char* data)
{
	...
	if(peer.player == NULL) {
		...
		sprintf(fname, "logss\\JPG_%d_%d_%d_%s_%x.jpg", ginfo_.gameServerId, peer.player->profile_.CustomerID, peer.player->loadout_->LoadoutID,peer.player->userName, GetTickCount());
	}
	...
	char chatmessage[128] = {0};
	PKT_C2C_ChatMessage_s n;
	sprintf(chatmessage, "FairFuck Banned '%s' ",peer.player->loadout_->Gamertag);
	r3dscpy(n.gamertag, "<System>");
	r3dscpy(n.msg, chatmessage);
	n.msgChannel = 1;
	n.userFlag = 2;
