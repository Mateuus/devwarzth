...
void InitTs3Server()
{
	ts3server_createVirtualServer(9987, "0.0.0.0", "warzthvoipserver", "", 32, &svHandlerID);
...
void Get_Camera_Bob(r3dPoint3D& camBob, r3dPoint3D& camUp, const obj_Player* player)
{
	...
	if(g_camera_mode->GetInt()==2)
	{
		...
		case PLAYER_SWIM:// intentional fallthrough
			{
				boba *= .6f; // yeah this doesn't do anything, it's here in case we want to tweak it. 
				rolla = 1;
			}
			break;
		case PLAYER_SWIM_M:// intentional fallthrough
			{
				boba *= .6f; // yeah this doesn't do anything, it's here in case we want to tweak it. 
				rolla = 1;
			}
			break;
		case PLAYER_SWIM_F:// intentional fallthrough
			{
				boba *=1.0f; 
				rolla = 2;
			}
		...
		/*obj_Player* pl = gClientLogic().localPlayer_;
		if (pl->isDriving() && pl->curcar)
		{
		boba *=pl->curcar->vd->vehicle->computeForwardSpeed()/15; 
		rolla = pl->curcar->vd->vehicle->computeForwardSpeed()/15;
		}
		boba *= 3;
		rolla = 2;*/
...
void ProcessPlayerMovement(obj_Player* pl, bool editor_debug )
{
	...
	VMPROTECT_BeginMutation("ProcessPlayerMovement_Accel");	
	{
		...
		if(!disablePlayerMovement)
		{
			...
			{
				if(InputMappingMngr->isPressed(r3dInputMappingMngr::KS_MOVE_FORWARD) || shiftWasPressed || d_autorun->GetBool() )  // you cheating!
				{
