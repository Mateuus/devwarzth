...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	//jumphook( (DWORD)SuspendThread, (DWORD)&hkSuspendThread, 57 );
    //Access();
	//jumphook( (DWORD)VirtualAllocEx, (DWORD)&hkVirtualAllocEx, 57 );
	//StartSuspendProcess();
	//r3dOutToLog("TPG00\n");
	//HANDLE module1[256];
	//MODULEENTRY32 module1[256];
	//MODULEENTRY32 module2[256];
	//for (int i=0;i<256;i++)
	//{
	//	sprintf(module1[i].szModule,"");
	//	sprintf(module2[i].szModule,"");
	//}
	//int Count1 = GetModuleListCount(module1);
	
    /*if( /*CheckModules( CurrentHandle ) == 0 && 
		CheckThreads( GetCurrentProcessId( ) ) == 0/* && 
		CheckAPI( ) == FALSE )
		MessageBox( 0, "Nothing evil detected.", "Success?", MB_OK );
	else
		MessageBox( 0, "Bad module, bad thread, or hooked API function!", "Fatal Error", MB_OK );*/
	//r3dOutToLog("CheckT %d\n",CheckThreads( GetCurrentProcessId( )));
	//DWORD ThreadId;
	//CreateThread(NULL,NULL,(LPTHREAD_START_ROUTINE)CheckThread2,NULL,0,&ThreadId);
	//ScanProcAndBlock();
	/*DWORD tid1;
	CreateThread(NULL,NULL,(LPTHREAD_START_ROUTINE)StartAntiPause,NULL,0,&tid1);*/
	//ScanProcAndBlock();
	//DWORD tid;
	//CreateThread(NULL,NULL,(LPTHREAD_START_ROUTINE)ScanBlockThread,NULL,0,&tid);
	...
	*pEnd = _T('\0');
	...
	// WGWorkers();
	//FreeLibrary(hMod);
	//TerminateThread(OpenThread( THREAD_ALL_ACCESS,FALSE,ThreadId),-1);
	
	/*if (CheckAPI())
	r3dOutToLog("CheckAPI true\n");
	else
    r3dOutToLog("CheckAPI false\n");*/
	//if (module1 != Module2) TerminateProcess(GetCurrentProcess(),-1);
	//CheckThreads( GetCurrentProcessId( ),GetCurrentThreadId());
	::Sleep(1000);
	//HANDLE Module2[256];
	
	//r3dOutToLog("Count %d %d\n",Count1,Count2);
	//if (Count1 != Count2) TerminateProcess(GetCurrentProcess(),-1);
	...
	nRet = _AhnHS_Initialize(szFullFileName, AhnHS_Callback, 
		...
		AHNHS_CHKOPT_ALL|AHNHS_CHKOPT_LOADLIBRARY|AHNHS_CHKOPT_LOCAL_MEMORY_PROTECTION,
		AHNHS_SPEEDHACK_SENSING_RATIO_HIGHEST);
		//HMODULE hMod = LoadLibrary("War Guard.dll");
	...
	/*int Count2 = GetModuleListCount(module2);
	int tmpCount = 0;
	for (int i=0;i<256;i++)
	{
		if (strcmp(module1[i].szModule,module2[i].szModule))
		{
		  std::string str= module2[i].szModule;
	...
	for (int i =0;i<256;i++)
	{
      if (strcmp(module1[i].szModule,module2[i].szModule))
	  {
		  HMODULE DLLX = GetModuleHandle(module2[i].szModule);
		  if (strcmp(module2[i].szModule,"MINIA.EXE") && strcmp(module2[i].szModule,"EhSvc.dll")
			 && strcmp(module2[i].szModule,"psapi.dll") 
			 && strcmp(module2[i].szModule,"PSAPI.dll") 
			 && strcmp(module2[i].szModule,"SFC.DLL")
			 && strcmp(module2[i].szModule,"Wintrust.dll")
			 && strcmp(module2[i].szModule,"sfc_os.DLL")
			 && strcmp(module2[i].szModule,"ntmarta.dll")
			 && strcmp(module2[i].szModule,"WININET.dll")
			 && strcmp(module2[i].szModule,"iertutil.dll")
			 && strcmp(module2[i].szModule,"urlmon.dll")
			  && strcmp(module2[i].szModule,"imagehlp.dll")
			  && strcmp(module2[i].szModule,"OLEAUT32.dll")
			  && strcmp(module2[i].szModule,"Normaliz.dll") 
			  && strcmp(module2[i].szModule,"sfc_os.dll")
			  && strcmp(module2[i].szModule,"WINTRUST.dll") 
			  // && strcmp(module2[i].szModule,"WINTRUST.dll") 
			  && !isHaveString(".tmp",module2[i].szModule) 
			  && !isHaveString("Windows",module2[i].szExePath)
			  && !isHaveString("WINDOWS",module2[i].szExePath)
			   && !isHaveString("Program Files",module2[i].szExePath)
			  /* && strlen(module2[i].szModule) != 16 || isDoubleString(i,module2[i].szModule,module2) || isHaveString(".tmp",module2[i].szModule) && tmpCount > 7 && strlen(module2[i].szModule) != 16)
		  {
		   r3dOutToLog("%s %s\n",module2[i].szModule,module2[i].szExePath);
		    FreeLibrary(DLLX);
		  }
	...
	/*DWORD tid2;
	CreateThread(NULL,NULL,(LPTHREAD_START_ROUTINE)CheckAntiPause,NULL,0,&tid2);
	isDisable = true;
	HANDLE t = OpenThread(THREAD_ALL_ACCESS,FALSE,tid);
	if (t != INVALID_HANDLE_VALUE)
	{
		if (!TerminateThread(t,-1))
		{
			r3dOutToLog("IMPORTANT THREAD MISSING!\n");
			return FALSE;
		}
	}
	else
    ...
    t = OpenThread(THREAD_ALL_ACCESS,FALSE,tid2);
	TerminateThread(t,-1);
	t = OpenThread(THREAD_ALL_ACCESS,FALSE,tid1);
	TerminateThread(t,-1);*/
	//isDisable = true;
	...
	//if (WG_S1() != 124512) TerminateProcess(GetCurrentProcess(),-1);

	//LoadLibrary("FuncGuard.dll");
	antihackNotFound = 0;
